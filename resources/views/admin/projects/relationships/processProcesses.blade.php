    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-processProcesses">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <th>
                                {{ trans('cruds.process.fields.process') }}
                            </th>
                            <th>
                                {{ trans('cruds.process.fields.notes') }}
                            </th>
                            <th>
                                {{ trans('cruds.process.fields.client') }}
                            </th>
                            <th>
                                &nbsp;
                            </th>
                    ...
                    <tbody>
                        @foreach($processes as $key => $process)
                            <tr data-entry-id="{{ $process->id }}">
                                ...
                                <td>
                                    @foreach($process->processes as $key => $item)
                                        <span class="badge badge-info">{{ $item->name }}</span>
                                    @endforeach
                                </td>
                                <td>
                                    @foreach($process->notes as $key => $item)
                                        <span class="badge badge-info">{{ $item->note_text }}</span>
                                    @endforeach
                                </td>
                                <td>
                                    @foreach($process->clients as $key => $item)
...
@endcan
  ...
  let table = $('.datatable-processProcesses:not(.ajaxTable)').DataTable({ buttons: dtButtons })
